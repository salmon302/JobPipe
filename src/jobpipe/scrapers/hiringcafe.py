...
from jobpipe.scrapers.selector_profiles import HIRINGCAFE_SELECTOR_PROFILE
...
class HiringCafeScraper:
    def __init__(self, config: HiringCafeScraperConfig) -> None:
        self._config = config
        self._selectors = HIRINGCAFE_SELECTOR_PROFILE
    ...
    async def scrape(self, max_pages: int = 1) -> list[JobRecord]:
        ...
        async with async_playwright() as playwright:
            ...
            if auth_status.usable:
                ...
                if self._config.require_usable_auth_state:
                    raise UnusableStorageStateError(
                        "HiringCafe scraper requires a usable storage state, but none was found at "
                        f"{self._config.storage_state}: {issue_summary}. "
                        "Run `jobpipe auth-bootstrap --platform hiringcafe` first."
                    )
            ...
            # Standard playwright automation signals are often blocked. 
            # We can try to add some basic evasions here if needed later.
            ...
            # Navigate to base URL first to establish session/cookies if not already there
            try:
                LOGGER.info("Navigating to base URL to establish session...")
                await page.goto(self._config.base_url, wait_until="domcontentloaded", timeout=30000)
                await asyncio.sleep(2)
            except Exception as e:
                LOGGER.warning("Base URL navigation failed: %s", e)

            for next_url in search_urls:
                LOGGER.info("Starting scrape for URL: %s", next_url)
                for _ in range(max_pages):
                    try:
                        # Hiring Cafe Cloudflare is very sensitive to 'networkidle'.
                        # Stick with 'domcontentloaded' but verify we aren't stuck on a challenge.
                        await page.goto(next_url, wait_until="domcontentloaded", timeout=60000)
                    except Exception as e:
                        LOGGER.warning("Initial goto failed for %s: %s. Retrying with 'load' and longer timeout...", next_url, e)
                        await page.goto(next_url, wait_until="load", timeout=90000)

                    LOGGER.info("Current URL after goto: %s", page.url)
                    
                    # Detect and wait for Cloudflare challenge (max 60s)
                    content = await page.content()
                    if any(term in content for term in ["Cloudflare", "cf-browser-verification", "Ray ID"]) or "security verification" in content.lower():
                        LOGGER.warning("Cloudflare challenge or protection detected on %s. Waiting for resolution (max 60s)...", page.url)
                        for i in range(1, 21): # 20 * 3s = 60s
                            await asyncio.sleep(3)
                            # Perform small mouse movements to look more 'human'
                            try:
                            ...
                            content = await page.content()
                            if not any(term in content for term in ["Cloudflare", "Ray ID"]) and "security verification" not in content.lower():
                                LOGGER.info("Challenge resolved after %ds.", i * 3)
                                # Wait a bit more for the real content to hydrate
                                await asyncio.sleep(5)
                                content = await page.content()
                                break
                            
                            # Or if cards appear
                            if await page.query_selector(self._selectors.card_selector):
                                LOGGER.info("Cards appeared during challenge wait.")
                                break
                            LOGGER.info("Still waiting for challenge (attempt %d/20)...", i)
                    
                    LOGGER.info("Page content length: %d", len(content))
                    
                    # Try to capture the body text or some indicators
                    body_text = await page.inner_text("body")
                    LOGGER.info("Body text snippet (first 200 chars): %s", body_text[:200].replace('\n', ' '))

                    # Wait for results or no-results indicator
                    try:
                        # Hiring Cafe cards often take a while to hydrate
                        await page.wait_for_selector(self._selectors.card_selector, timeout=20000)
                        LOGGER.info("Cards found after waiting")
                    except Exception:
                        LOGGER.warning("No cards found after 20s wait. Check if search returned no results or if selectors changed.")
                        # Check for 'No jobs found' text
                        if "No jobs found" in body_text or "No results" in body_text:
                            LOGGER.info("Confirmed: Search returned no results.")
                        else:
                            LOGGER.info("Neither cards nor 'No results' text found. Might be stuck.")
                    ...
                    LOGGER.info("Parsed %d jobs from page", len(jobs))
                    for job in jobs:
    ...
    async def _parse_page(self, page: Page) -> list[JobRecord]:
        ...
        LOGGER.info("Found %d raw card matches", len(cards))
    ...
    async def _fetch_detail_description(self, page: Page, job_url: str) -> str:
        ...
        try:
            ...
            LOGGER.debug("Failed to enrich job description from %s: %s", job_url, exc)
        finally:
    ...
    def _build_job_id(self, node_id: str | None, url: str) -> str:
        return build_job_id("hiringcafe", node_id=node_id, url=url)
