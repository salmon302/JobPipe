...
from jobpipe.scrapers.selector_profiles import BUILTIN_SELECTOR_PROFILE
...
class BuiltInScraper:
    def __init__(self, config: BuiltInScraperConfig) -> None:
        self._config = config
        self._selectors = BUILTIN_SELECTOR_PROFILE
            ...
            if auth_status.usable:
                ...
                if self._config.require_usable_auth_state:
                    raise UnusableStorageStateError(
                        "Built In scraper requires a usable storage state, but none was found at "
                        f"{self._config.storage_state}: {issue_summary}. "
                        "Run `jobpipe auth-bootstrap --platform builtin` first."
                    )
    ...
    async def _fetch_detail_description(self, page: Page, job_url: str) -> str:
        ...
        try:
            ...
            LOGGER.debug("Failed to enrich Built In description from %s: %s", job_url, exc)
        finally:
    ...
    def _build_job_id(self, node_id: str | None, url: str) -> str:
        return build_job_id("builtin", node_id=node_id, url=url)
